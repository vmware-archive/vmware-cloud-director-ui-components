...
import { ANGULAR_CORE, ROUTE } from './constants';
...
export function getRoutes(program: ts.Program): AppRoute[] {
    ...
    const moduleRoutesWithLazyLoading: ModuleRoutes[] = fixupLoadChildren(moduleRoutes, moduleFiles);

    // Get only the routes into a single array
    const appRoutes: AppRoute[] = moduleRoutesWithLazyLoading
...
function routeCallToRoutes(moduleRouteCall: ModuleRouteCall, typeChecker: ts.TypeChecker): ModuleRoutes {
    ...
    function convert(routeCallExpression: ts.Expression): AppRoute[] {
        ...
        const appRoutes: AppRoute[] = routeObjectLiteralExpressions.map(serializeRoute);

        // The end.
        return appRoutes;
    }

    function serializeRoute(expr: ts.ObjectLiteralExpression): AppRoute {
        ...
        expr.properties.forEach(prop => {
            ...
            } else if (propName === ROUTE.REDIRECT_TO) {
                route.redirectTo = evaluateNode(prop.initializer, typeChecker);
            } else if (propName === ROUTE.CHILDREN) {
            ...
            } else if (propName === ROUTE.LOAD_CHILDREN) {
            ...
            } else if (propName === ROUTE.COMPONENT) {
                // Maybe not really needed, but just in case we add the component name
                route.component = prop.initializer.getText();
            } else if (propName === ROUTE.CAN_ACTIVATE) {
                // Maybe not really needed, but just in case we add the guard name
                route.canActivate = prop.initializer.getText();
            } else if (propName === ROUTE.DATA) {
...
function fixupLoadChildren(
    ...
    const moduleRoutes: ModuleRoutes[] = moduleRoutesWithLoadChildren.map(moduleRoute => {
        const routes = moduleRoute.routes.map(route => {
            ...
            const referencedModule = getReferencedModule(route.loadChildren);
