...
### Changed
- Enabled `TestElement` to query children elements by css selector
### Removed
...
### Changed
- BREAKING: Rewrote `VcdDatagridWidget` and `ClrDatagridWidget` with new Widget Object.
- Added convenience methods to the `TestElement`.
- INTERNAL: Rewrote `datagrid.component` unit tests using new widget object.
- INTERNAL: Rewrote helper method about creating datagrid filter unit test with new Widget Object.
### Removed
- BREAKING: Removed `WithGridBoldRenderer`. The functionality of `WithGridBoldRenderer` has been implemented in `ClrDatagridWidget`.
### Deprecated
- Marked `WidgetFinder` as deprecated.
...
- BREAKING: Fixes an error in the type clash with Jasmine and Chai in the Widget Object
- Fix arrow navigation of disabled menu items.
- INTERNAL: Replace DropdownItem with ActionMenuItem


## [1.0.0-dev.48] - 2020-10-30
- BREAKING: Removes the old WidgetObject
- BREAKING: Reworks the interface of the new WidgetObject to use a system of locators
        Documentation can be found at the [pull request](https://github.com/vmware/vmware-cloud-director-ui-components/pull/227)
- BREAKING: Removed the deprecated `ButtonConfig` interface. This has been replaced by the `ActionItem` interface.
- Remove spaces surrounding the label in FormInputComponent and FormSelectComponent
- Add ARIA role to the ErrorBannerComponent
- Prevent sub-menus that are opened on hover from closing when clicked
